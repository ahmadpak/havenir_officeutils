...
def execute(filters=None):
	...
	data = []
	res_len = len(res)
	res[0]['reconciled'] = 'Opening'
	res[res_len - 2]['reconciled'] = 'Total'
	res[res_len - 1]['reconciled'] = 'Closing (Opening + Total)'
	for row in res:
		if row.posting_date:
			row['reconciled'] = frappe.get_value(row.voucher_type,row.voucher_no,'reconciled')
			row['reconciled_by'] = frappe.get_value(row.voucher_type,row.voucher_no,'reconciled_by')
		data.append(row)
	return columns, data
